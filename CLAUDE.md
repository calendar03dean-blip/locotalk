...
## 현재 상태 (2026-06-12 기준 · v1.0.0 build 45)

> 직전 문서는 build 4 시점이라 build 5~45 개발분을 아래로 현행화함.

### 빌드 이력
- ✅ App Store 심사 제출 (v1.0.0 build 2) / TestFlight build 4 (소셜 로그인)
- ✅ build 41 — 코드네임·약관게이트 포함 로컬 아카이브 → Transporter 전송
- ✅ build 42~44 — 본인인증 테스트 진입 우회 / 지역경계 / 코드네임 / 이메일계정 반영 (일부 ASC stale로 소진)
- ✅ **build 45 (현재 HEAD `8c9e38a`, buildNumber=45) — TestFlight VALID** (2026-06-12 14:29 KST 업로드, 로그인 UX B안 포함). 추가 bump 불필요.

### 인증·계정 (구조 전환됨)
- ✅ **A안: 이메일 + 비밀번호 계정 + 본인인증(PortOne) 1회** — 계정 생성 권위 단일화 (`a815912`)
- ✅ **로그인 UX B안** — 본인인증 회원가입을 메인 主CTA로, 이메일/비번 로그인은 숨김 레이어 (`1b0edba`)
- ✅ 진입 이탈 방지 동의 카피/레이아웃 리파인 (`de1cfaa`)
- ✅ 본인인증 실연동 전 **테스트 진입 우회** + A/B 신원 분리(1:1 채팅 device e2e 가능) (`d1ca96c`, `a031bac`)
- (소셜 로그인 4종/이메일 OTP 코드는 잔존하나, 진입 동선은 본인인증 중심으로 이동)

### 온보딩·진입 게이트
- ✅ **랜덤 코드네임 시스템** — 온보딩 닉네임 입력 대체 (조용한너구리 #7F3A 형식, 서버 검증·중복 409)
- ✅ 코드네임 9~14자 join_queue 거부 봉합 (`a8ed331`)
- ✅ **약관 3종 동의 게이트 + 위치 권한 필수 게이트** (진입 차단형) — 위탁 다날(Danal) 확정 + 버전 태그 (`56d498b`, `6b56f4d`, `e679322`)
- ✅ 동의 증빙 영속 — 로컬 큐 1차 + 경계 재시도/flush + agreed_at 드리프트 봉합 (`505948b`, `49d3051`)
- ✅ 개인정보처리방침 **만 19세 전용**으로 모순 봉합 (ko/en, docs+server 동기화) (`24824a6`)
- ✅ `/terms`·`/location-terms`·`/privacy` 404 봉합 (server/ 빌드 컨텍스트로 이동) (`331fbed`)

### 지역·매칭
- ✅ **전국 시군구 경계 데이터** 배치 + resolver name 매핑 (옵션 A 활성화) (`c39706e`)
- ✅ 매칭 알고리즘 (관심사 점수 + 거리 필터 + 프리미엄 우선)
- ✅ 지역 선택 피커 (프리미엄)

### 출시 안전가드·자동화
- ✅ appstore 빌드 + `IDENTITY_LIVE=false` 제출 물리 차단 / submit:ios 제출가드 (`c876493`, `27190ea`)
- ✅ handoff·long-task·alert 프로토콜 CLAUDE.md 명문화 + ASC archive 권한 (`007b814`)
- ✅ EAS/IPA 산출물·ASC .p8 키(secret)·카드 캐시 gitignore (`bacd17e`)

### 기존 유지
- ✅ 인앱 결제 (react-native-iap) / 채팅 저장 프리미엄 전용 / 신고 기능

### 🔄 진행 중 / 미해결
- 본인인증(PortOne) 실연동 (현재 테스트 우회 상태)
- 약관 본문 변호사 최종 검토 (현재 다날 위탁 반영 버전)
- 온보딩(코드네임/약관게이트) 실기기 e2e 최종 확인
- origin/main 대비 로컬 1커밋 ahead(`8c9e38a`) — push는 대표 GO 시
