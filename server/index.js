...
function createRoom(socketA, userA, socketIdB, userB,
  ...
  const peerForA = { nick: userB.nick, interests: userB.interests, region: userB.region, roomId };
  const peerForB = { nick: userA.nick, interests: userA.interests, region: userA.region, roomId };

  socketA.emit(eventA,       { roomId, peer: peerForA });
  io.to(socketIdB).emit(eventB, { roomId, peer: peerForB });

  console.log(`[match] ${userA.nick} ↔ ${userB.nick}  room=${roomId}  [${eventA}/${eventB}]`);
  return true;
}
...
function tryMatch(socket, user) {
  ...
  const sameRegion = candidates.filter(([, c]) => c.region === user.region);
...
io.on('connection', (socket) => {
  ...
  socket.on('join_queue', (user) => {
    ...
    console.log(`[queue] +${entry.nick} (${entry.region})  queue=${queue.size}`);
  ...
  socket.on('send_message', ({ roomId, text }) => {
    ...
    // 항상 push 발송 — 화면 꺼짐/백그라운드/앱 종료 모두 커버
    // 클라이언트 setChatFocused 가 채팅 포커스 중 배너 표시를 막음
    if (otherUser?.nick) {
