...
async function initDB() {
  ...
  try {
    ...
    try {
      const warm = await feeds.loadAllForWarming(db);
...
app.post('/users/:id', async (req, res) => {
  ...
  // [하드닝] 표시명은 자동 생성 코드네임만 수용 — 허용 단어셋+hex 패턴 외 임의 문자열 거부.
  //   nickname 미동봉(지역만 갱신 등)이면 검증 스킵(COALESCE 로 기존값 유지).
  if (nickname != null) {
    if (!codename.isValidCodename(nickname)) {
      return res.status(400).json({ error: 'invalid_codename' });
    }
    try {
      const dup = await db.query('SELECT 1 FROM users WHERE nickname = $1 AND id <> $2 LIMIT 1', [nickname, id]);
      if (dup.rows.length) return res.status(409).json({ error: 'codename_taken' });
    } catch (e) { /* 유일성 조회 실패는 무시(형식검증은 이미 통과) */ }
  }
...
if (DEBUG_DISABLED) {
  ...
  app.get('/debug/reports', async (req, res) => {
    ...
    const list  = await reports.listReports(db, { limit: req.query.limit || 50 });
    res.json({ ...stats, items: list });
  });

  // 동네 피드 영속 적재 카운트(재시작 생존 확인용)
...
io.on('connection', (socket) => {
  ...
  // ── join_region (동네 피드) ──────────────────────────────────────
  socket.on('join_region', async ({ region } = {}) => {
    ...
    // 접속자에게 피드 히스토리 전송 (최신순 최대 100). DB 우선 → 실패/없음 시 인메모리 캐시 폴백.
    let history = await feeds.loadRegion(db, region, 100);
    if (!history || history.length === 0) history = regionFeeds.get(region) || [];
    else regionFeeds.set(region, history); // 캐시 신선화
    if (history.length > 0) {
  ...
  socket.on('post_feed', ({ text, nick, interest, region } = {}) => {
    ...
    // 인메모리 저장 (최신순, 최대 100개) + 파일 영속화(레거시)
    ...
    // DB 영속화(best-effort, fire-and-forget) — 재시작 생존. INSERT 후 region당 100 초과분 정리.
    feeds.insertFeed(db, { id: item.id, region, nick, item })
      .then(() => feeds.pruneRegion(db, region, 100))
      .catch(() => {});
  ...
  socket.on('report_user', ({ reportedNick, roomId, reason, reporterNick } = {}) => {
    ...
    // 영속화(best-effort, 실패해도 위 신고 흐름 무중단).
    // 스냅샷은 '서버가' room.events 로 구성(클라 스냅샷 미신뢰). 이미지 payload 는 '[image]' 치환(크기 제한).
    try {
      const room = rooms.get(roomId);
      const events = (room?.events || chatLogs.get(roomId) || []).slice(-20).map(e => ({
        kind: e.kind,
        at  : e.at,
        text: e.kind === 'image' ? '[image]' : String(e.payload ?? '').slice(0, 500),
      }));
      const reportedUserId = room?.users?.find(u => u.nick === reportedNick)?.userId || null;
      reports.insertReport(db, {
        reporterId    : socket.userId || null,
        reportedUserId,
        reportedNick,
        conversationId: roomId || null,
        reason,                         // insertReport 가 화이트리스트로 정규화
        snapshot      : JSON.stringify(events),
      });
    } catch (e) {
      console.warn('[report] persist 실패(무시):', e.message);
    }
  });
...
app.delete('/admin/api/feeds/:region/:id', (req, res) => {
  ...
  saveData();
  feeds.deleteFeed(db, region, id).catch(() => {}); // DB 동기화(best-effort)
